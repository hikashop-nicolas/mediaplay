...
function browserLacksAudioCodec(codec, probe) {
...
async function remuxWithAudioTrack(blob, keepTrackId) {
...
class MediaPlayer {
    ...
    wrap = null;
    url = null;
    srcBlob = null; // disk-backed source for playback/remux (no full copy in RAM)
    eagerBytes = null; // set only when the caller passed bytes directly
    materialized = null; // buffer read from srcBlob on demand, cached
    media = null;
    applyLiveSubtitle = null;
    onDocKey = null;
    subUrls = [];
    teardown = [];
    decodedAudio = null;
    ...
    mount(container, source) {
        ...
        this.srcBlob = source.blob ?? (source.bytes && source.bytes.length ? new Blob([source.bytes], mime ? { type: mime } : undefined) : null);
        ...
        if (this.srcBlob) {
            ...
            if (savedRate && savedRate !== 1)
                m.addEventListener("loadeddata", () => setRate(savedRate, false), { once: true });
            // Assigned in the video-only tracks section below; C toggles subtitles.
            let toggleSubs = () => undefined;
            const toggleFullscreen = () => {
                if (document.fullscreenElement)
                    void document.exitFullscreen();
                else
                    void wrap.requestFullscreen?.().catch(() => undefined); // wrap, so subs/overlays come along
            };
            const fail = () => {
                wrap.textContent = "";
            ...
            m.addEventListener("error", () => {
                ...
                tryRemux(srcBlob, isAudio).then((blob) => {
                    note.remove();
                    if (!blob)
                        return fail();
                    if (this.url)
            ...
            this.onDocKey = (e) => {
                ...
                if (!wrap.isConnected)
                    return;
                if (wrap.checkVisibility ? !wrap.checkVisibility() : wrap.offsetParent === null && !document.fullscreenElement)
                    return;
            ...
            const pullFocus = () => {
                if (wrap.isConnected && !wrap.contains(document.activeElement))
                    wrap.focus();
            };
            ...
            if (!isAudio) {
                ...
                let libassFonts = [...(this.opts.libass?.fonts ?? [])];
                let octopus = null;
                let octopusFor = -1;
                const dropOctopus = () => {
                    try {
                ...
                const showVttFallback = (i) => {
                    if (activeSub !== i)
                        return;
                    const entry = subTracks[i];
                    if (!entry.el)
                        entry.el = attachTrackEl(entry);
                ...
                const poke = () => {
                    ...
                    if (document.fullscreenElement === wrap)
                        idleTimer = window.setTimeout(() => {
                            if (menu.hidden)
                                wrap.classList.add("ot-media-idle");
                        }, 2500);
                };
                ...
                m.addEventListener("dblclick", (e) => {
                    ...
                    if (e.clientY > r.bottom - 70)
                        return; // over the native control bar
                    e.preventDefault();
                    toggleFullscreen();
                });
                ...
                fileInput.addEventListener("change", async () => {
                    ...
                    try {
                        ...
                        addSubTrack({ label: f.name.replace(/\.[^.]+$/, ""), lang: "und", vtt, assDoc: isAss ? decodeSubtitleBytes(raw) : undefined }, true);
                    }
                    catch {
                        /* unreadable subtitle file */
                    }
                });
                const attachTrackEl = (entry) => {
                ...
                const setSub = (i) => {
                    ...
                    subTracks.forEach((entry, j) => {
                        if (entry.el)
                            entry.el.track.mode = j === i && !styled ? "showing" : "disabled";
                    });
                    if (target && !styled && !target.el) {
                        target.el = attachTrackEl(target);
                        target.el.track.mode = "showing";
                    }
                    if (target && styled && octopusFor !== i)
                        void startOctopus(i);
                    rebuildMenu();
                };
                toggleSubs = () => {
                    if (subTracks.length)
                        setSub(activeSub >= 0 ? -1 : Math.min(lastSub, subTracks.length - 1));
                };
                const addSubTrack = (entry, select) => {
                    subTracks.push({ ...entry, el: null });
                    if (select)
                        setSub(subTracks.length - 1);
                    else
                        rebuildMenu();
                };
                ...
                this.applyLiveSubtitle = (content, filename) => {
                    ...
                    if (assDoc && octopusFor === liveIndex && live?.setTrack) {
                        ...
                        if (m.paused) {
                            ...
                            if (typeof live.lastRenderTime === "number")
                                live.lastRenderTime = -1;
                ...
                const switchAudio = async (i) => {
                    ...
                    m.addEventListener("loadeddata", () => {
                        ...
                        if (!wasPaused)
                            ...
                            m.pause(); // autoplay would otherwise restart a paused player
                    }, { once: true });
                    activeAudio = i;
                    rebuildMenu();
                };
                const rebuildMenu = () => {
                    ...
                    subTracks.forEach((entry, i) => item(entry.label || `#${i + 1}`, activeSub === i, () => {
                        setSub(i);
                        menu.hidden = true;
                    }));
                ...
                window.setTimeout(async () => {
                    ...
                    try {
                        ...
                        for (const f of info.fonts) {
                            const url = URL.createObjectURL(new Blob([f.data], { type: f.mime || "font/otf" }));
                        ...
                        if (activeCodec && browserLacksAudioCodec(activeCodec, m)) {
                            if (/^A_(E?AC3|DTS|TRUEHD|MLP)$/i.test(activeCodec))
                                void this.startDecodedAudio(m, activeAudio, showToast, directFor(activeAudio));
                            else
                                showToast(S.mediaAudioUnsupported);
                        }
                    }
                    catch {
    ...
    async startDecodedAudio(video, audioIndex, showToast, direct) {
        ...
        try {
            ...
            if (handle && handle !== "undecodable")
                ...
                showToast(this.S.mediaAudioUnsupported);
        }
        catch (e) {
    ...
    setSubtitleText(content, filename) {
        this.applyLiveSubtitle?.(content, filename);
    }
    focus() {
        this.wrap?.focus?.();
 ...
 */
export function createMediaPlayer(container, source, opts = {}) {
