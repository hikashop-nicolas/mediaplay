...
export async function* readAudioPacketsFromBlob(blob: Blob, trackNumber: number, fromMs: number, info: MkvInfo): AsyncGenerator<MkvAudioPacket> {
  if (!info.clusters.length) return;
  let startIdx = 0;
  for (let i = 0; i < info.clusters.length; i++) {
    if (info.clusters[i]!.timeMs <= fromMs) startIdx = i;
    else break;
  }
  for (let ci = startIdx; ci < info.clusters.length; ci++) {
    ...
    try {
      while (r.pos < clusterEnd) {
        ...
        if (!blk || blk.track !== trackNumber) continue;
        const tsMs = ((clusterTs + blk.relTime) * info.timestampScale) / 1e6;
        if (tsMs < fromMs - 100) continue;
        for (const [off, len] of blk.frames) {
          if (len > 0 && off + len <= buf.length) yield { tsMs, data: buf.subarray(off, off + len) };
        }
      }
    } catch {
