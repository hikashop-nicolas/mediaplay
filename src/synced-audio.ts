 ...
 */
function mkvDirectSource(src: Uint8Array | Blob, trackNumber: number, ffCodec: string, base: string, info: MkvInfo): AudioChunkSource {
...
export async function decodeAudioToMono16k(
  blob: Blob,
  opts: { base?: string; audioIndex?: number; signal?: AbortSignal; onProgress?: (ratio: number) => void; durationHint?: number } = {},
  ...
  if (!track) throw new Error("no audio track to transcribe");
  ...
  try {
    for await (const { buffer, timestamp } of source.buffers(0, ctx)) {
      ...
      for (let c = 0; c < nch; c += 1) {
        ...
        for (let i = 0; i < len; i += 1) mono[i] += ch[i]! / nch;
      }
  ...
  if (!total) throw new Error("audio track produced no samples");
